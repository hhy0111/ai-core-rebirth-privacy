...
- 2026-05-10 Play Console versionCode 중복 대응: Play Console에서 `versionCode 1`이 이미 사용되었다고 표시되어 `app/build.gradle.kts`의 `versionCode`를 2, `versionName`을 `0.1.1`로 상향. `testDebugUnitTest` 통과, `bundleRelease` 재생성, bundle manifest에서 `android:versionCode="2"`와 `android:versionName="0.1.1"` 확인. 업로드 키로 `app-release-upload-signed.aab`를 다시 서명했고 SHA256은 `973536C8F75CCE2078B2A9EEDCDC067AE628C7A1C4EFAFE35EFB37502BFD5C6A`.

- 2026-05-10 광고 제거/스킨 상품 체감 개선: `remove_ads` 보유 시 보상형 광고 버튼, 이동 탭 항로 보급, 지나가는 조우 오브젝트 보상을 광고 없이 즉시 지급하도록 연결하고 즉시 보상 쿨다운을 추가. 이동 탭에는 `TRAVEL_SUPPLY`, 조우 오브젝트에는 `ENCOUNTER_CLAIM` 보상형 광고 배치를 추가했으며, 조우 보상은 3배/균열 복원 보상은 2배로 강화. `planet_skin_pack` 보유 시 상점 상품 행을 탭해 기본/네온 생명/산업 코어/전투 균열 스킨을 순환 적용하고, 선택 스킨을 저장하도록 추가. 새 스킨 이미지 프롬프트는 `docs/art/IMAGE_PROMPTS_ADDED_PLANET_SKINS_AND_REWARDED_ADS_2026-05-10.md`에 분리 작성. 신규 광고 단위 ID `rewarded_travel_supply = ca-app-pub-4402708884038037/9476869104`, `rewarded_encounter_claim = ca-app-pub-4402708884038037/5727371953` 적용 완료. 검증: `./gradlew.bat compileDebugKotlin`, `./gradlew.bat testDebugUnitTest` 통과. TODO: 스킨 PNG 3종을 생성해 `drawable-nodpi`에 추가.

- 2026-05-10 행성 스킨 이미지 적용 및 출시 AAB 재생성: `image/`의 `01-planet_skin_life_neon.png`, `02-planet_skin_industrial_forge.png`, `03-planet_skin_combat_void.png`를 각각 `app/src/main/res/drawable-nodpi/planet_skin_life_neon.png`, `planet_skin_industrial_forge.png`, `planet_skin_combat_void.png`로 적용하고 사용한 원본 3개를 삭제. 변경 AAB 업로드 충돌 방지를 위해 `versionCode = 3`, `versionName = 0.1.2`로 상향. 검증: `./gradlew.bat testDebugUnitTest`, `./gradlew.bat bundleRelease`, 업로드 키 `local/ai-core-rebirth-upload.jks` 서명 및 `jarsigner -verify` 통과. 새 업로드 파일은 `app/build/outputs/bundle/release/app-release-upload-signed.aab`, SHA256 `189277A2B4795A3D3240D79344EAD4E56B9FE683B43F1095182FA86682F0ECAD`.
